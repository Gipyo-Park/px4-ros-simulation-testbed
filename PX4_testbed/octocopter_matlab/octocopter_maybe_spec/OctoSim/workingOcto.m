...
PHI_INPUT_SATURATION = deg2rad(30);
THETA_INPUT_SATURATION = deg2rad(12);
% -------------------------------------------------------------------------
% drone parameter
% -------------------------------------------------------------------------
load('quadModel_X');
...
ct = 5.837e-06;
cq = 4.49581e-09; %ct / 1.4865e-07 * 2.9250e-09;
...
% Kz = 12*quadModel.ct*quadModel.cr*(quadModel.cr*GO + quadModel.b);
% num_Gz = Kz;
% den_Gz = [m 0 0];
% Gz = tf(num_Gz,den_Gz);
...
Go_z_approx = Gc_z*Gz;
Gcl_z_approx = feedback(Go_z_approx, 1);
...
simulation = sim('PC_Quadcopter_Simulation_Alchemist_angleSafety');
...
plot(time, rad2deg(Psid_sim), 'black', 'linewidth', 2);
...
plot(time, W_cmd, 'black', 'linewidth', 2);
plot(time, W, 'blue-.', 'linewidth', 1.5);
title('W'), xlabel('time [s]'), ylabel('w [m/s]')
legend('Reference Input','Output')