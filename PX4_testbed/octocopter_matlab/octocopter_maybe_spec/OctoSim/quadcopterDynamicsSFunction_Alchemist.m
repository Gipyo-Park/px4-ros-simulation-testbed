...
function setup(block)
  ...
  block.InputPort(9).Dimensions        = 6; % torques x,y,z; forces x,y,z.
...
%------
Dist_tau = block.InputPort(9).Data(1:3);
Dist_F   = block.InputPort(9).Data(4:6);
%------
...
%                 CW: +, CCW: -                                % CW: -, CCW: +
 tau_motorGyro = [Q*quad.Jm*2*pi/60*(w1+w2-w3-w4-w5-w6+w7+w8); P*quad.Jm*2*pi/60*(-w1-w2+w3+w4+w5+w6-w7-w8); 0]; % Note: 2*pi/60 required to convert from RPM to radians/s
  Mb = (quad.dctcq*(w.^2)) + tau_motorGyro + (Dist_tau);  % Mb = [tau1 tau2 tau3]'
% Mb = (quad.dctcq*(w.^2)) + (Dist_tau);
...
RHO = 8.6; %8.6; %1, 3.4, 8.6;
r = 0.356; %0.05; %quad.cq/quad.ct;  % rotor radius
% A = 2*r*0.01; % rotor area
Zr = 0.28; %0.05; % rotor height

Fb = [0; 0; sum(quad.ct*(w.^2))];   %[0, 0, sum(ct*w.^2)]'
Fb = Fb./(1 - RHO*(r/(4*(Z + Zr)))^2);

% uh = sqrt(Fb(3)/(2*A*rho));
% Fb = Fb./((1 - 3*r/(25*(Z+Zr)))/(1 - (3/50)*(U/uh)^3) - (29/250)*(U/uh)^3);
...
%     dW = 0;
    ...
    dY = 0; %
    dZ = 0; %
%     block.ContStates.Data(4) = 0; % Phi
%     block.ContStates.Data(5) = 0; % Theta
%     block.ContStates.Data(7) = 0; % U
%     block.ContStates.Data(8) = 0; % V
%     block.ContStates.Data(9) = 0; % W
    block.ContStates.Data(12) = 0;% Z
end
