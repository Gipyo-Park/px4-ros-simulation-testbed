...
class SweepLemniscate:
    ...
    def get_next_pose(self):
        ...
        # 3. 3D 위치 계산: 3D Lissajous Curve
        # X는 3번 진동할 때 Y는 2번 진동 -> 복잡한 매듭 모양 형성
        # Z는 1번 진동 -> 입체적인 움직임
        x = self.cur_radius * math.sin(3 * t)
        y = self.cur_radius * math.sin(2 * t)
        z = self.cur_z_base + self.cur_z_amp * math.sin(t)

        # 4. Yaw(헤딩) 계산: 진행 방향 바라보기 (속도 벡터 미분)
        # x' = 3 * R * cos(3t)
        # y' = 2 * R * cos(2t)
        dx = 3 * self.cur_radius * math.cos(3 * t)
        dy = 2 * self.cur_radius * math.cos(2 * t)
