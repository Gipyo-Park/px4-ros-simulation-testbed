...
class RealtimePatchTSTComparison:
    def __init__(self, checkpoint_path, seq_len=100, pred_len=25, patch_len=100, stride=6):
        """
        Args:
        ...
        self.log_dir = f"/home/hmcl/PX4_testbed/octocopter_matlab/drone_metric_matlab/prediction_logs/{self.model_type}/pred_len_{pred_len}_patch_len_{patch_len}_stride_{stride}"
    ...
    def load_model(self, checkpoint_path):
        ...
        configs = type('Args', (), {
            ...
            'fc_dropout': 0.3,
            'head_dropout': 0.0,
            'patch_len': self.patch_len,       # Patch length
            'stride': self.stride,           # Patch stride
            'individual': False,
            'revin': True,
            'affine': False,
            'subtract_last': False,
    ...
    def perform_prediction(self):
        ...
        try:
            ...
            # 예측
            with torch.no_grad():
                # PatchTST 입력: (batch, seq_len, n_vars)
                # PatchTST 출력: (batch, pred_len, n_vars)
                output = self.model(input_tensor)  # (1, pred_len, 15)
            
            # 역정규화
            output_numpy = output.cpu().numpy()
            predicted_all = output_numpy[0] * self.std + self.mean  # (pred_len, 15)
...
if __name__ == '__main__':
    try:
        ...
        checkpoint_path = '/home/hmcl/Downloads/PatchTST-main/PatchTST-main/PatchTST_supervised/checkpoints/drone_position_pred_100_75_pl32_str8_PatchTST_custom_ftM_sl100_ll48_pl75_dm128_nh16_el6_dl1_df512_fc1_ebtimeF_dtTrue_Exp_0/checkpoint.pth'
        
        # 비교 시스템 초기화
        comparator = RealtimePatchTSTComparison(
            checkpoint_path=checkpoint_path,
            seq_len=100,
            pred_len=75,  # checkpoint와 동일한 pred_len 사용
            patch_len=32,
            stride=8
        )
