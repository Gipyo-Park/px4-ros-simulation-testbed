function [FCMW,avg_array_FCM] = calculate_FCMW(FCM,window_size_)
    ...
    persistent array_FCM currentCount maxWindow;
    
    % 최초 호출 시 고정 크기 배열을 할당하고, 카운터를 0으로 초기화.
    if isempty(array_FCM)
        maxWindow = window_size_;  % 최대 배열 크기를 window_size_로 정함.
        array_FCM = zeros(1, maxWindow);  % 고정 크기 배열 할당
        currentCount = 0;
    end
    ...
    if currentCount < window_size_
        ...
        array_FCM(currentCount) = FCM;
    else
        array_FCM(1:window_size_-1) = array_FCM(2:window_size_);
        array_FCM(window_size_) = FCM;
    end
    
    % 채워진 요소만 고려하여 평균 계산
    avg_array_FCM = sum(array_FCM(1:currentCount)) / currentCount;
    
    % FCMW 계산: 평균이 0.5 미만이면 0, 아니면 1로 설정
