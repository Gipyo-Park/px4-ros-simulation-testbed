...
{ // 가장 큰 바운더리
  ...
  */
  ...
  { // gazebo 바운더리 안에 GazeboMotorModel 바운더리
  public:
    GazeboMotorModel()
  ...
  private:
    ...
    // gazebo::physics::JointPtr 형태이다
    // gazebo::physics::boost::shared_ptr<Joint>
    // joint_ = model_->GetJoint(joint_name_); 여기서 joint_ 를 얻어온다.
