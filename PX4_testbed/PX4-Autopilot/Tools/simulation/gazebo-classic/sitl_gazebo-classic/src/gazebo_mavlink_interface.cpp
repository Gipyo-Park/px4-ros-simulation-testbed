...
namespace gazebo
{
  ...
  GazeboMavlinkInterface::GazeboMavlinkInterface() : ModelPlugin()
  {
    ...
    // mavlink_interface_는 GazeboMavlinkInterface 클래스 내에서 사용되는 포인터로, MavlinkInterface 클래스의 인스턴스를 가리킵니다.
    // GazeboMavlinkInterface는 Gazebo 시뮬레이터와 MAVLink 프로토콜을 사용하는 비행 컨트롤러(PX4와 같은) 간의 통신을 관리하는 역할을 합니다.
    // mavlink_interface_는 그 자체로 MAVLink 메시지의 송수신, 처리, 그리고 비행 컨트롤러와의 상호작용을 담당합니다.
    // 이 포인터를 통해 GazeboMavlinkInterface 클래스는 MavlinkInterface 클래스의 함수들을 호출하여 MAVLink 메시지를 처리하고,
    // Gazebo 시뮬레이션에서 드론의 상태나 제어 명령을 처리할 수 있습니다.
  ...
  /*
  실제 동작 흐름 예시
  센서 데이터 수신: Gazebo의 특정 센서에서 메시지가 구독됨.
  callback() 호출: 이 구조체의 callback() 함수가 실행됨.
  콜백 전달: ptr로 가리키는 GazeboMavlinkInterface 객체의 멤버 함수 fp가 호출되고, 메시지와 센서 ID가 전달됨.
  센서 데이터 처리: 전달된 메시지와 센서 ID를 기반으로 센서 데이터를 처리함.
  */
  template <typename GazeboMsgT>
  struct SensorHelperStorage
  {
    /// \brief    Pointer to the ROS interface plugin class.
    // 콜백 함수가 호출될 때 이 포인터를 사용해 GazeboMavlinkInterface 클래스의 멤버 함수에 접근합니다.
    GazeboMavlinkInterface *ptr;

    /// \brief    Function pointer to the subscriber callback with additional parameters.
    // fp: 콜백 함수 포인터로, 구독된 메시지를 처리하는 데 사용할 멤버 함수입니다.
    ...
    // Gazebo는 구독 시 한 개의 인자(메시지 데이터)만 전달할 수 있기 때문에, boost::bind()와 같은 방식으로 여러 인자를 처리하는 것이 불가능합니다.
    // 이를 해결하기 위해, 구독된 메시지와 **추가 파라미터(sensor_id)**를 콜백 함수에 전달하기 위해 이 헬퍼 구조체가 사용됩니다.
    //  예를 들어, 센서 데이터가 수신되면 이 callback() 함수가 호출되고, 메시지 데이터와 센서 ID가 실제 콜백 함수로 전달됩니다.
  ...
  // CreateSensorSubscription 함수는 Gazebo 시뮬레이터의 센서 구독을 쉽게 설정하기 위한 헬퍼 함수입니다. 여러 개의 센서에 대한 구독(subscription)을 생성하고, 센서의 방향을 설정하는 등의 작업을 처리합니다.
  template <typename GazeboMsgT>
  ...
  // CreateSensorSubscription 함수는 안봐도 될듯
  // ----------------------------------** 여기서 부터 보면 될듯------------------
  void GazeboMavlinkInterface::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
  {
    ...
    // protocol_version 버전은 2.0으로 되어있따.
    ...
    {
      pids_[i].Init(0, 0, 0, 0, 0, 0, 0); // pids_[index].Init(p, i, d, iMax, iMin, cmdMax, cmdMin); 이런 순서다
    ...
    if (_sdf->HasElement("control_channels"))
    {
      ...
      while (channel)
      {
        ...
        {
          int index = channel->Get<int>("input_index"); // sdf 파일에서 input_index랑 n_out_max랑 비교, 만약 false이면 에러 뜸
          if (index < n_out_max)
          {
            ...
            if (channel->HasElement("joint_name"))
            {
              ...
              // std::cout << "joint_name HasElement:  " << joint_name << std::endl;
              // iris일경우 이렇게 하면 joint_name에 joint_name HasElement:  zephyr_delta_wing::propeller_joint
              // joint_name HasElement:  zephyr_delta_wing::flap_left_joint
              // joint_name HasElement:  zephyr_delta_wing::flap_right_joint
              // 이런게 담기는데
              // GetJoint(zephyr_delta_wing::flap_right_joint) 하면 당연 이런 이름의 조인트가 없다.
              // 그래서 joints_[index] 에 0이 담긴다.

              //  iris, octocopter 둘다  <!-- <joint_name>rotor_1_joint</joint_name> -->  이렇게 sdf파일 수정하면 if문 들어온다.
              // 이런식으로 sdf 수정하면 joints_[index]에 값이 생긴다.
              joints_[index] = model_->GetJoint(joint_name);
              // std::cout << "index:  " << index << std::endl;
              // std::cout << "joints_[index]:  " << joints_[index] << std::endl;
            }
    ...
    // ------------------------** input_reference_ 는 여기서 부터 여기까지 보기 ----------------
    ...
    // **updateConnection_**는 Gazebo의 시뮬레이션 갱신 이벤트(매 프레임마다 발생)를 처리하는 콜백 함수(OnUpdate)를 설정합니다.
    updateConnection_ = event::Events::ConnectWorldUpdateBegin(
        boost::bind(&GazeboMavlinkInterface::OnUpdate, this, _1));

    // Listen to Ctrl+C / SIGINT.
    // **sigIntConnection_**는 SIGINT 이벤트(Ctrl+C로 시뮬레이션이 종료될 때)를 처리하는 콜백 함수(onSigInt)를 설정합니다.
    ...
    // gazebo_groundtruth_plugin 에서 오는 토픽인데, 이 코드는 Gazebo 시뮬레이터에서 Groundtruth 데이터를 가져와 이를 MAVLink 또는 다른 시스템에 전송하는 플러그인이다.
    ...
    gazebo_controller_interface 하고 gazebo_mavlink_interface 에서만 /gazebo/command/motor_speed 이런 토픽을 publish하고 있다
    하지만 gazebo_controller_interface 는 플러그인으로 사용안되고 잇기 때문에 신경 안써도 된다.
    즉 gazebo_mavlink_interface 만 publish하고 있다.
    */
    // motor_velocity_reference_pub_ 가 보내는 메세지 내용은 turning_velocities_msg 인데
    // turning_velocities_msg 에는 제어신호가 스케일링 된값이다.
    // 스케일링 안된값을 보고 싶으면 actuator_controls 값을 출력해보길
  ...
  void GazeboMavlinkInterface::OnUpdate(const common::UpdateInfo & /*_info*/)
  {
    ...
    if (imu_received_once_)
    {
      ...
      {
        imu_received_cond_.wait_for(lock, std::chrono::milliseconds(10));
      }
    }
    imu_received_ = false;
    ...
    // mavlink_message_t msg 에 sensor_msg를 전부 담는다.
    // sensor_msg 에는 imu,mag, baro, diff 의 센서 데이터가 담겨있고, 이거를 mavlink를 통해 send한다.
    SendSensorMessages();

    // Send groundtruth at full rate
    // SendGroundTruth 함수에서 생성된 hil_state_quat 값들은 가제보에서 얻은 드론의 실제 데이터다. measurement같은 센서 데이터가 아니다.
    // 당연 prediction 데이터도 아니다. 왜냐하면 Get~* 함수들을 사용하는거보니 가제보에서 그대로 가져오는값인것 같다.
    SendGroundTruth();
    ...
    handle_control(dt); // 내가 sdf파일에 잘 설정하면 실행되는 함수.
    ...
    if (received_first_actuator_)
    {
      ...
      for (int i = 0; i < input_reference_.size(); i++)
      {
        ...
        {
          // 여기는 들어오지못하고 else부분만 접근한다.
          turning_velocities_msg.add_motor_speed(0);
        }
        else
        {
          turning_velocities_msg.add_motor_speed(input_reference_[i]);
          // std::cout << "turning_velocities_msg[" << i << "]: " << turning_velocities_msg.motor_speed(i) << "  a = " << a++ << std::endl;
        }

        // turning_velocities_msg 값이랑  input_reference_ 값이랑 같네. 출력하는 방법만 다른듯 왜냐하면 타입이 다르니깐
        // std::cout << "turning_velocities_msg[" << i << "]: " << turning_velocities_msg.motor_speed(i) << "  a = " << a++ << std::endl;
      }
  ...
  void GazeboMavlinkInterface::ImuCallback(ImuPtr &imu_message)
  {
    ...
    imu_received_ = true;
    imu_received_once_ = true;
    lock.unlock();
    imu_received_cond_.notify_one();
  }
...
// ground truth
#if GAZEBO_MAJOR_VERSION >= 9
    ignition::math::Pose3d pose_gr = model_->WorldPose();
#else
    ...
    // std::cout << " check hil_state_quat value = " << hil_state_quat.zacc << "  a = " << a++ << std::endl;
  ...
  void GazeboMavlinkInterface::handle_actuator_controls()
  {
    ...
    // GetActuatorControls는 mavlink_interface.cpp 파일에 정의 되어 있다.
    // 내 느낌은 mavlink_interface_ 이거 덕분에 mavlink_interface.cpp에 함수를 이용할수 있는듯?
    // 즉 actuator_controls = input_reference_ 이 된다.
    ...
    for (int i = 0; i < input_reference_.size(); i++)
    {
      if (armed)
      {
        // 내 느낌 : input_reference_ 값이 normalized 된 값이여서 [-1 1] 범위 안에 값을 갖는다.
        // 모터마다 계수가 전부 다르기 때문에 input_scaling_ 값이나 offset등등 우리가 알아서 정하거나, 수식을 바꿔도 된다.
        // 실제 모터가 어느정도의 속도에서 나는 알면 좋을듯?
        input_reference_[i] = (actuator_controls[input_index_[i]] + input_offset_[i]) * input_scaling_[i] + zero_position_armed_[i];
        // std::cout << "input_reference_[" << i << "]: " << input_reference_[i] << "  a = " << a << std::endl;
      }
      else
      {
    ...
    이렇게 iris에 정의 되어잇는데 여기서 input_offset을 0으로 해야 5번째 신호에 0이 된다.
    */
  ...
  void GazeboMavlinkInterface::handle_control(double _dt)
  {
    ...
    for (int i = 0; i < input_reference_.size(); i++)
    {
      // std::cout << "handle_control:  " << joints_[i] << std::endl;

      // std::cout << "input_reference_.size:  " << input_reference_.size() << std::endl;
      // std::cout << "input_reference_[" << i << "]: " << input_reference_[i] << "  a = " << a++ << std::endl;

      if (joints_[i] || joint_control_type_[i] == "position_gztopic")
      {
        // std::cout << "hhhhhh  " << a++ << std::endl;
        //  iris, octocopter 둘다  <!-- <joint_name>rotor_1_joint</joint_name> -->  이렇게 sdf파일 수정하면 if문 들어온다.
