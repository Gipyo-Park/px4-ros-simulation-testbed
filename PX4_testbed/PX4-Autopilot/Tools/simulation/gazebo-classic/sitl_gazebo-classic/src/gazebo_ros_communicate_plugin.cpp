...
namespace gazebo
{
    ...
    void MotorVelocityPlugin::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
    {
        ...
        rosNode = new ros::NodeHandle(this->namespace_);
        my_motor_velocity_pub = rosNode->advertise<std_msgs::Float64>("/my/" + model_->GetName() + "/motor_speed/" + std::to_string(communicate_motor_Number), 10);
