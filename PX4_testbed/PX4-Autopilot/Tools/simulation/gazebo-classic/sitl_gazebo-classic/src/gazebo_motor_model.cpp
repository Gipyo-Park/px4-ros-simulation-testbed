...
namespace gazebo
{
  ...
  void GazeboMotorModel::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
  {
    ...
    */
    ...
    getSdfParam<double>(_sdf, "maxRotVelocity", max_rot_velocity_, max_rot_velocity_); // maxRotVelocity 는 rad/s 이다
    ...
    */
    ...
    // motor_force_pub_ = node_handle_->Advertise<physics_msgs::msgs::Force>("~/" + model_->GetName() + motor_speed_pub_topic_, 1);  // 이부분 수정했다
  ...
  */
  ...
  void GazeboMotorModel::VelocityCallback(CommandMotorSpeedPtr &rot_velocities)
  {
    ...
    else
    {
      ...
      // std::cout << "rot_velocities = " << static_cast<double>(rot_velocities->motor_speed(motor_number_)) << " a = " << a << std::endl;
      // std::cout << "max_rot_velocity_ = " << static_cast<double>(max_rot_velocity_) << " a = "<< a << std::endl;
    }
  ...
  void GazeboMotorModel::UpdateForcesAndMoments()
  {
    ...
    // real_motor_velocity : (rad/s) : 실제 모터 속도가 21 이면 real_motor_velocity 는 현재 210이다.

    // std::cout << " motor angular velocity [rad/s] = " << real_motor_velocity / 10  << " a = " << a++ << std::endl;
    // Ct 는 무차원 : thrust coefficient
    // F= ma   // [N] = [kg][m/s^2]
    // motor_constant_ : [kg*m] : [N*s^2/rad^2] 
    // std::cout << "motor_constant_ = " << motor_constant_ << "  a = " << a << std::endl;
    // std::cout << "real_motor_velocity = " << real_motor_velocity << "  a = " << a << std::endl;
    ...
    // std::cout << "force = " << force << "  a = " << a << std::endl;
    ...
    /*
      로터축과 평행한 수직으로 오는 바람 고려해서, drag Force계산
    */
    ...
    /*
    이렇게 (scalar = 1 - vel / 25.0) 스케일링 하는 이유를 설명한다.
    드론이 하버링 할때는 움직이지 않으니깐 공기저항을 적게 받어. 하지만 드론이 저속으로 이동하면 공기저항을 조금이라도 받어. 그리고 드론이 고속으로 이동하면 공기저항을 많이 받어.
    이처럼 드론의 속도가 빨라질수록 공기저항이 쌔져서 드론의 thrust가 줄어드는데 이거를 모델링해서 나타낼려면 어렵기 때문에 스케일링을 통해서 아주 쉽게 표현한거다.
    저속일때는 이와 같은 방법은 영향을 많이 주지 않는다.
    하지만 고속일때 경향을 잘 보여주기에는 적합할수도 있는 방법이다.
    */
    ...
    // std::cout << "AddRelativeForce [" << ignition::math::Vector3d(0, 0, force * scalar) << "] a = " << a << std::endl;

    /*
      로터축과 수직인 평행하게 오는 바람 고려해서, drag Force계산
    */
    ...
    // velocity_perpendicular_to_rotor_axis : 로터 축에 수직한 바람의 속도 벡터. 
    ...
    // std::cout << "air_drag [" << air_drag << "] a = " << a << std::endl;
    ...
    */
    ...
    // std::cout << "drag_torque_parent_frame = " << drag_torque_parent_frame << " a = " << a << std::endl;
    ...
    */
    ...
    // std::cout << "rolling_moment [" << rolling_moment << "] a = " << a++ << std::endl;
  ...
  else
  {
    ...
    joint_->SetVelocity(0, turning_direction_ * ref_motor_rot_vel / rotor_velocity_slowdown_sim_);
    // std::cout << "SetVelocity = " << turning_direction_ * ref_motor_rot_vel / rotor_velocity_slowdown_sim_ << " a = " << a << std::endl;
